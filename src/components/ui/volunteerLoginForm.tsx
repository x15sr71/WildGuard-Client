...
async function callBackendLogin(idToken: string) {
  try {
    const response = await fetch("https://wildgaurd-backend-794553988056.asia-south1.run.app/auth/login", {
      method: "POST",
      headers: {
...
export function VolunteerLoginForm() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const profileResponse = await fetch(`https://wildgaurd-backend-794553988056.asia-south1.run.app/volunteer-profile`, {
        method: "GET",
